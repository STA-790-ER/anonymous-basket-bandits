...
if idx == 1
    ...
    for t in 1:T
        ...
        @timeit to "train_$(t)" Flux.train!(loss, Flux.params(m), [(X,Y)], opt)

        @timeit to "save" @save "/hpc/home/jml165/rl/valneuralnets/valnn_$(t)_$(idx).bson" m
    end
    end
    ...
    #@timeit to "load" @load "/hpc/home/jml165/rl/valneuralnets/valnn_$(t)_$(idx-1).bson" m
    m = Chain(Dense(ncol - 2, hidden_size, σ), Dense(hidden_size, 1))
    loss(x,y) = sum(Flux.Losses.mse(m(x),y))
    Flux.train!(loss, Flux.params(m), [(X,Y)], opt)

    #@timeit to "save" @save "/hpc/home/jml165/rl/valneuralnets/valnn_$(t)_$(idx).bson" m
    #@timeit to "rm" rm("/hpc/home/jml165/rl/valneuralnets/valnn_$(t)_$(idx-1).bson")

    while true
        if isfile("/hpc/home/jml165/rl/valneuralnets/valnn_$(T)_$(idx-1).bson")
             @timeit to "loop" begin
             for t in 1:T
...
if idx == 399
    for t in 1:T
        ...
        @save "/hpc/home/jml165/rl/valneuralnets/valnn_$(t).bson" m
        rm("/hpc/home/jml165/rl/valneuralnets/valnn_$(t)_$(idx).bson")
