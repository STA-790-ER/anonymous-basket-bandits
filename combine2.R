temp = list.files(path="~/rl/arrayresults2", pattern=glob2rx("results*.csv"), full.names=TRUE)
agreetemp = list.files(path="~/rl/arrayresults2", pattern = glob2rx("agree*.csv"), full.names = TRUE)
...
write.csv(agree_prop, file = "~/rl/combresults2/agreeprop.csv")
...
write.csv(file_means, file="~/rl/combresults2/combresults.csv", row.names=TRUE)
write.csv(file_vars, file="~/rl/combresults2/combvars.csv", row.names=TRUE)
...
pdf(file="~/rl/combresults2/regretplot.pdf")
ploty
dev.off()
