...
function vb_bernoulli_ep_contextual_bandit_simulator(ep,action_function, T, rollout_length, n_episodes, n_rollouts, n_opt_rollouts, context_dim, context_mean,
    context_sd, obs_sd, bandit_count, bandit_prior_mean, bandit_prior_sd, discount, epsilon, global_bandit_param)
        ...
        for t in 1:T
            ...
            obs = 1 * (ground_truth_obs_bern[t, ep] < true_expected_reward)
            Y_tot[t] = obs
            A_tot[t] = action
            X_tot[t, :] = context
...
function vb_bernoulli_contextual_bandit_simulator(action_function, T, rollout_length, n_episodes, n_rollouts, n_opt_rollouts, context_dim, context_mean,
    ...
    for ep in 1:n_episodes
        ...
        if use_context_prior
            EPREWARDS, EPOPTREWARDS = vb_bernoulli_ep_contextual_bandit_simulator_context_mvn(ep,action_function, T, rollout_length, n_episodes, n_rollouts, n_opt_rollouts, context_dim, context_mean,
                                       context_sd, obs_sd, bandit_count, bandit_prior_mean, bandit_prior_sd, discount, epsilon, global_bandit_param)
        else
...
function dp_valcheck_vb_bernoulli_val_greedy_thompson_ucb_ids_policy(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim)
...
## PREALLOCATION
    ...
    for k in 1:bandit_count
        if sum(A .== k) == 0
            BANDIT_PARAM_SAMPS[k, :, :] = rand(MvNormal(ms_Cs[k][1], ms_Cs[k][2]), n_opt_rollouts)
        else
            BANDIT_PARAM_SAMPS[k, :, :], ACC = sample_mcmc_vb(X[A .== k, :], y[A .== k], prior_mean, prior_cov, ms_Cs[k][1], ms_Cs[k][2], n_opt_rollouts, n_burn, n_dup)
            println("Acceptance Prob: ", ACC)
            flush(stdout)
        end
    end
...
function vb_bernoulli_val_rollout(ep, policy, T_remainder, rollout_length, context, use_context, lambda, context_dim, context_mean,
    ...
    for t in 1:min(T_remainder, rollout_length)
        ...
        true_expected_reward_logit = dot(bandit_param[action,:], context)
        true_expected_reward = exp(true_expected_reward_logit) / (1 + exp(true_expected_reward_logit))
        mer = 1 / (1 + exp(-maximum(bandit_param*context)))
        disc_reward += (true_expected_reward-mer) * discount^(t-1)
...
function dp_vb_rollout(ep, policy, T_remainder, rollout_length, dp_length, dp_actions, context, use_context, lambda, context_dim, context_mean,
    ...
    for t in 1:min_remainder_dp_length
        ...
        old_features = get_features_vb(context, ms_Cs, history)
        ...
        true_expected_reward = exp(true_expected_reward_logit) / (1 + exp(true_expected_reward_logit))
        mer = 1 / (1 + exp(-maximum(bandit_param*context)))
        push!(RESULTS[t], true_expected_reward-mer)
        ...
        new_features = get_features_vb(context, ms_Cs, history)
    ...
    for t in (1+min(T_remainder, dp_length)):min(T_remainder, rollout_length)
        ...
        true_expected_reward = exp(true_expected_reward_logit) / (1 + exp(true_expected_reward_logit))
        mer = 1 / (1 + exp(-maximum(bandit_param*context)))
        RESULTS[min_remainder_dp_length][3] += (true_expected_reward-mer) * discount^(t - min(T_remainder, dp_length))
        disc_reward += true_expected_reward * discount^(t-1)
...
function dp_valuation_vb_rollout(ep, policy, T_remainder, rollout_length, dp_length, dp_actions, context, use_context, lambda, context_dim, context_mean,
    context_sd, obs_sd, bandit_count, discount, X, y, A, bandit_param,
	true_expected_rewards, ms_Cs, models)
    ...
    for t in 1:min_remainder_dp_length
        ...
        if t == 1
            action = findmax(models[1])[2]
        else
            curr_features = get_features_vb(context, ms_Cs, history)
            action  = findmax([XGBoost.predict(models[t], reshape(reduce_features(curr_features, act), 1, length(curr_features)-(bandit_count-1)*3+bandit_count))[1] for act in 1:bandit_count])[2]
        end 
        #action = ms_Cs_policy(ep, t_curr, min(T_remainder, rollout_length), bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, ms_Cs)
        ...
        true_expected_reward_logit = dot(bandit_param[action,:], context)
        true_expected_reward = exp(true_expected_reward_logit) / (1 + exp(true_expected_reward_logit))
        max_expected_reward = 1 / (1+exp(-maximum(bandit_param * context)))
        #push!(RESULTS[t], true_expected_reward)
        #push!(RESULTS[t], 1 * (t == min_remainder_dp_length))
        disc_reward += (true_expected_reward-max_expected_reward) * discount^(t-1)
...
function get_features_vb_old(context, ms_Cs)
...
function get_features_vb(context, ms_Cs, history)
    input_vec = [dot(post[1],context) for post in ms_Cs]
    append!(input_vec, [sqrt(dot(context, a[2], context)) for a in ms_Cs])
    append!(input_vec, [sqrt(dot(context, a[2], context)) for a in ms_Cs] .* input_vec[1:bandit_count])
    append!(input_vec, context)
...
# Greedy
function vb_greedy_policy(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, args...)
...
function vb_greedy_policy_given_mC(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, ms_Cs, args...)
...
# Epsilon Greedy
function vb_epsilon_greedy_policy(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, args...)
...
# Bayes UCB
function vb_bayes_ucb_policy(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, args...)
    val, action = findmax(bandit_posterior_means * context)
...
# Bayes UCB
function vb_glm_ucb_policy(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, args...)
    #val, action = findmax(bandit_posterior_means * context)
...
function vb_glm_ucb_policy_given_mC(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, ms_Cs, args...)
    #val, action = findmax(bandit_posterior_means * context)
    reward_means = zeros(bandit_count)
    reward_sds = zeros(bandit_count)
    for k in 1:bandit_count
...
# Thompson
function vb_thompson_policy(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, args...)
...
function vb_thompson_policy_given_mC(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, ms_Cs, args...)
    thompson_means = zeros(bandit_count)
    for k in 1:bandit_count
        #m, C = jj_posterior_batch(X[A .== k,:], y[A .== k], prior_mean, prior_cov, .001)
...
function vb_ids_policy(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, args...)
...
function vb_ids_4_policy(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, args...)
...
function vb_ids_policy_given_mC(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, ms_Cs, args...)
...
function vb_ids_4_policy_given_mC(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, ms_Cs, args...)
...
# UNKOWN CONTEXT DISTRIBUTION


function vb_bernoulli_ep_contextual_bandit_simulator_context_mvn(ep,action_function, T, rollout_length, n_episodes, n_rollouts, n_opt_rollouts, context_dim, context_mean,
    context_sd, obs_sd, bandit_count, bandit_prior_mean, bandit_prior_sd, discount, epsilon, global_bandit_param)
        ...
        for t in 1:T
            ...
            context = ground_truth_context[ep][t]
            #println(context)
            #println(context_posterior)
            update_context_posterior!(context, context_posterior)
            #println(context_posterior)
            true_expected_rewards_logit = true_bandit_param * context
            true_expected_rewards = exp.(true_expected_rewards_logit) ./ (1 .+ exp.(true_expected_rewards_logit))
            #true_expected_rewards = bandit_posterior_means * context
            action = action_function(ep, t, T, bandit_count, context, X_tot, Y_tot, A_tot, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, context_posterior)
            true_expected_reward = true_expected_rewards[action]
            EPREWARDS[t] = true_expected_reward
            EPOPTREWARDS[t] = maximum(true_expected_rewards)
            #obs = randn() * sqrt(obs_sd^2 + dot(context, bandit_posterior_covs[action,:,:],context)) + true_expected_reward
...
function vb_bernoulli_val_greedy_thompson_ucb_ids_policy(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, context_posterior)
...
function vb_bernoulli_val_greedy_thompson_ucb_ids_q_policy(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, context_posterior)
...
function dp_vb_bernoulli_val_greedy_thompson_ucb_ids_policy(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, context_posterior)
...
function dp_valcheck_vb_bernoulli_val_greedy_thompson_ucb_ids_policy(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, context_posterior)
...
## PREALLOCATION
    ...
    for roll in 1:(n_opt_rollouts*vb_rollout_mult)
        ...
        for pol_ind in 1:length(policies)
            ...
            #println(ROLL_CONTEXT_DIST)
            rollout_counts[pol_ind] += 1
            bandit_param = rollout_params[:, 1 + ((roll - 1) % n_opt_rollouts), :]'
            
            rollout_value = vb_bernoulli_val_rollout(ep, policies[pol_ind], T-t+1, rollout_length, context, use_context, lambda, context_dim, context_mean,
                context_sd, obs_sd, bandit_count, discount, X, y, A, bandit_param,
                roll_true_expected_rewards, ms_Cs, ROLL_CONTEXT_DIST)
            A[t:T] .= 0
            REWARD_SAMPS[roll, pol_ind] = rollout_value
            #MEAN_REWARD = ((roll - 1) * MEAN_REWARD + rollout_value) / roll
    ...
    for tt in 1:dp_rollout_count
        ...
        ROLL_CONTEXT_DIST = MvNormal(CONTEXT_PARAM_SAMP[1], CONTEXT_PARAM_SAMP[2])
        RESULTS = dp_vb_rollout(ep, opt_policy, T-t+1, rollout_length, dp_length, dp_actions, context, use_context, lambda, context_dim, context_mean,
                             context_sd, obs_sd, bandit_count, discount, X, y, A, BANDIT_PARAM_SAMPS[:, :, tt], roll_true_expected_rewards, ms_Cs, context_posterior, ROLL_CONTEXT_DIST)
        A[t:T] .= 0
        for i in eachindex(RESULTS)
            if i == 1
                push!(MEM[i][dp_actions[1]], RESULTS[i])
    ...
    for s in 1:min(dp_length, T-t+1)
        tt = min(dp_length, T-t+1) - s + 1
        if tt>1
            Y = [u[3] + discount * (u[4] == 0 ? maximum([XGBoost.predict(M[tt+1], reshape(reduce_features_vb(u[5], bb), 1, length(u[5])-(bandit_count-1)*3+bandit_count))[1] for bb in 1:bandit_count]) : 0) for u in MEM[tt]]
            XX = reduce(vcat, transpose.([u[1] for u in MEM[tt]]))
            M[tt] = xgboost_cv(XX, Y, 4)
        else
            for b in 1:bandit_count
                Y = [u[3] + discount * (u[4] == 0 ? maximum([XGBoost.predict(M[tt+1], reshape(reduce_features_vb(u[5], bb), 1, length(u[5])-(bandit_count-1)*3+bandit_count))[1] for bb in 1:bandit_count]) : 0) for u in MEM[tt][b]]
                M[tt][b] = mean(Y)
            end
        end
    end
    ...
    for roll in 1:n_opt_rollouts  
        ...
        if !halt_vec[1]
            ...
            rollout_value = vb_bernoulli_val_rollout(ep, opt_policy, T-t+1, rollout_length, context, use_context, lambda, context_dim, context_mean, context_sd, obs_sd, bandit_count, discount, X, y, A, BANDIT_PARAM_SAMPS[:, :, roll], roll_true_expected_rewards, ms_Cs, ROLL_CONTEXT_DIST)
            A[t:T] .= 0
        ...
        if !halt_vec[2]
            ...
            rollout_value = dp_valuation_vb_rollout(ep, opt_policy, T-t+1, rollout_length, dp_length, dp_actions, context, use_context, lambda, context_dim, context_mean,
                                                 context_sd, obs_sd, bandit_count, discount, X, y, A, BANDIT_PARAM_SAMPS[:, :, roll], roll_true_expected_rewards, ms_Cs, M, context_posterior, ROLL_CONTEXT_DIST)
            A[t:T] .= 0 
            REWARD_SAMPS[roll, 2] = rollout_value     
        end
    ...
    REWARD_SAMPS = nothing
    if opt_index == 1
        return opt_policy(ep, t, T, bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim) 
    end
    return findmax(M[1])[2]
...
function vb_bernoulli_val_rollout(ep, policy, T_remainder, rollout_length, context, use_context, lambda, context_dim, context_mean,
    context_sd, obs_sd, bandit_count, discount, X, y, A, bandit_param,
	true_expected_rewards, ms_Cs, context_distribution)
    ...
    for t in 1:min(T_remainder, rollout_length)
        ...
        if t > 1 || !use_context
            context = generate_context_mvn(context_distribution, context_constant)
        end
        ...
        true_expected_reward_logit = dot(bandit_param[action,:], context)
        true_expected_reward = exp(true_expected_reward_logit) / (1 + exp(true_expected_reward_logit))
        maximum_expected_reward = 1 / (1 + exp(-maximum(bandit_param*context)))
        disc_reward += (true_expected_reward-maximum_expected_reward) * discount^(t-1)
...
function dp_vb_rollout(ep, policy, T_remainder, rollout_length, dp_length, dp_actions, context, use_context, lambda, context_dim, context_mean,
    context_sd, obs_sd, bandit_count, discount, X, y, A, bandit_param,
	true_expected_rewards, ms_Cs, context_posterior, context_distribution)
    ...
    RESULTS = [[] for i in 1:min_remainder_dp_length]
    history = []
    context_posterior = deepcopy(context_posterior)
    for t in 1:min_remainder_dp_length
        ...
        old_features = get_features_vb_unknown_context(context, ms_Cs, history, context_posterior)
        ...
        true_expected_reward = exp(true_expected_reward_logit) / (1 + exp(true_expected_reward_logit))
        maximum_expected_reward = 1 / (1 + exp(-maximum(bandit_param*context)))
        push!(RESULTS[t], true_expected_reward - maximum_expected_reward)
        push!(RESULTS[t], 1 * (t == min_remainder_dp_length))
        disc_reward += true_expected_reward * discount^(t-1)
        ...
        context = generate_context_mvn(context_distribution, context_constant)
        update_context_posterior!(context, context_posterior) 
        new_features = get_features_vb_unknown_context(context, ms_Cs, history, context_posterior)
	    push!(RESULTS[t], new_features)  
        # UNKNOWN PARAM VERSION
    end
    ...
    for t in (1+min(T_remainder, dp_length)):min(T_remainder, rollout_length)
        ...
        true_expected_reward = exp(true_expected_reward_logit) / (1 + exp(true_expected_reward_logit))
        maximum_expected_reward = 1 / (1 + exp(-maximum(bandit_param*context)))
        RESULTS[min_remainder_dp_length][3] += (true_expected_reward-maximum_expected_reward) * discount^(t - min(T_remainder, dp_length))
        disc_reward += true_expected_reward * discount^(t-1)
...
function dp_valuation_vb_rollout(ep, policy, T_remainder, rollout_length, dp_length, dp_actions, context, use_context, lambda, context_dim, context_mean,
    context_sd, obs_sd, bandit_count, discount, X, y, A, bandit_param,
	true_expected_rewards, ms_Cs, models, context_posterior, context_distribution)
    ...
    for t in 1:min_remainder_dp_length
        ...
        if t == 1
            ...
            curr_features = get_features_vb_unknown_context(context, ms_Cs, history, context_posterior)
            action  = findmax([XGBoost.predict(models[t], reshape(reduce_features(curr_features, act), 1, length(curr_features)-(bandit_count-1)*3+bandit_count))[1] for act in 1:bandit_count])[2]
        end 
        #action = ms_Cs_policy(ep, t_curr, min(T_remainder, rollout_length), bandit_count, context, X, y, A, discount, epsilon, rollout_length, n_rollouts, n_opt_rollouts, context_dim, ms_Cs)
...
function get_features_vb_unknown_context(context, ms_Cs, history, context_posterior)
    input_vec = [dot(post[1],context) for post in ms_Cs]
    append!(input_vec, [sqrt(dot(context, a[2], context)) for a in ms_Cs])
    append!(input_vec, [sqrt(dot(context, a[2], context)) for a in ms_Cs] .* input_vec[1:bandit_count])
    append!(input_vec, context_posterior[1])
    append!(input_vec, context_posterior[2])
    append!(input_vec, context_posterior[3])
    append!(input_vec, upper_triangular_vec(context_posterior[4]))
    append!(input_vec, context)
    for item in history
        append!(input_vec, item)
    end
