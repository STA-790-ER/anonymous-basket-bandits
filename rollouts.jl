...
function get_features_unknown_context(context, bandit_posterior_means, bandit_posterior_covs, context_posterior)
    input_vec = bandit_posterior_means * context
    append!(input_vec, [sqrt(dot(context, bandit_posterior_covs[a,:,:], context)) for a in 1:bandit_count])
    append!(input_vec, [sqrt(dot(context, bandit_posterior_covs[a,:,:], context)) for a in 1:bandit_count] .* (bandit_posterior_means * context))
    append!(input_vec, Vector(vec(bandit_posterior_means')))
...
function reduce_features_old(features, action)
    ...
    one_hot[action] = 1.
    return vcat(features[vcat(action_features, rem_features)], one_hot) 
            ...
            if true 
                ...
                for n in 1:50
                    ...
                    if String(Symbol(policy)) == "thompson_policy"
                        ...
                        #for m in 1:500
                        #reg_est *= ((n-1)*500 + m - 1) / ((n-1)*500 + m)
                        reg_est += (max_reward - expected_rewards[action]) / 50
                        #end
                    end
                    #reg_est *= (n-1) / n
                    #reg_est += (findmax(point_samps)[1] - point_samps[action]) / n
...
function dp_valuation_rollout(ep, policy, T_remainder, rollout_length, dp_length, dp_actions, context, use_context, lambda, context_dim, context_mean,
    context_sd, obs_sd, bandit_count, discount, bandit_posterior_covs, bandit_posterior_means, bandit_param,
	true_expected_rewards, CovCon, old_cov, SigInvMu, models)
...
#    RESULTS = [[] for i in 1:min_remainder_dp_length]
    ...
    for t in 1:min_remainder_dp_length
        ...
        if t == 1
            ...
            curr_features = get_features_2(context, bandit_posterior_means, bandit_posterior_covs)
...
#	    push!(RESULTS[t], new_features)  
        ...
        for t in (1+min(T_remainder, dp_length)):min(T_remainder, rollout_length)
            ...
            #TRUE PARAM VERSION
            true_expected_reward = dot(bandit_param[action, :], context)
            disc_reward += (true_expected_reward - maximum(bandit_param * context)) * discount^(t - 1)
            obs = randn() * obs_sd + true_expected_reward
            old_cov .= bandit_posterior_covs[action, :, :]
            mul!(CovCon, old_cov, context)
            CovCon ./= obs_sd
        ...
        if truncation_length > 0
            ...
            if true 
                ...
                disc_reward -= .9 * discount^(rollout_length) * sum(discount^(t - t_trunc) * reg_est * (1 + T - t) / (1 + T - t_trunc) for t in t_trunc:T)
...
function val_rollout(ep, policy, T_remainder, rollout_length, context, use_context, lambda, context_dim, context_mean,
    context_sd, obs_sd, bandit_count, discount, bandit_posterior_covs, bandit_posterior_means, bandit_param,
	true_expected_rewards, CovCon, old_cov, SigInvMu, ROLL_CONTEXT_DIST)
    ...
    for t in 1:min(T_remainder, rollout_length)
        ...
        if t > 1 || !use_context
            context = generate_context_mvn(ROLL_CONTEXT_DIST, context_constant)
        end
...
function dp_rollout(ep, policy, T_remainder, rollout_length, dp_length, dp_actions, context, use_context, lambda, context_dim, context_mean,
    context_sd, obs_sd, bandit_count, discount, bandit_posterior_covs, bandit_posterior_means, bandit_param,
	true_expected_rewards, CovCon, old_cov, SigInvMu, context_posterior, ROLL_CONTEXT_DIST)
    ...
    for t in 1:min_remainder_dp_length
        ...
        old_features = get_features_unknown_context(context, bandit_posterior_means, bandit_posterior_covs, context_posterior)
        ...
        context = generate_context_mvn(ROLL_CONTEXT_DIST, context_constant)
        update_context_posterior!(context, context_posterior)
        new_features = get_features_unknown_context(context, bandit_posterior_means, bandit_posterior_covs, context_posterior)
	    push!(RESULTS[t], new_features)  
...
function dp_valuation_rollout(ep, policy, T_remainder, rollout_length, dp_length, dp_actions, context, use_context, lambda, context_dim, context_mean,
    context_sd, obs_sd, bandit_count, discount, bandit_posterior_covs, bandit_posterior_means, bandit_param,
	true_expected_rewards, CovCon, old_cov, SigInvMu, models, context_posterior, ROLL_CONTEXT_DIST)
...
#    RESULTS = [[] for i in 1:min_remainder_dp_length]
    ...
    for t in 1:min_remainder_dp_length
        ...
        if t == 1
            ...
            curr_features = get_features_unknown_context(context, bandit_posterior_means, bandit_posterior_covs, context_posterior)
            action  = findmax([XGBoost.predict(models[t], reshape(reduce_features(curr_features, act), 1, length(curr_features)-(bandit_count-1)*3+bandit_count))[1] for act in 1:bandit_count])[2]
        end 
        #action = dp_actions[t]
        #action_ind = findfirst(arm_order .== action)
...
#	    push!(RESULTS[t], new_features)  
        ...
        for t in (1+min(T_remainder, dp_length)):min(T_remainder, rollout_length)
            ...
            #TRUE PARAM VERSION
            true_expected_reward = dot(bandit_param[action, :], context)
            disc_reward += (true_expected_reward - maximum(bandit_param * context)) * discount^(t - min(T_remainder, dp_length))
            obs = randn() * obs_sd + true_expected_reward
            old_cov .= bandit_posterior_covs[action, :, :]
            mul!(CovCon, old_cov, context)
            CovCon ./= obs_sd
        ...
        if truncation_length > 0
            ...
            if true 
                ...
                disc_reward -= .8 * discount^(1+min(T_remainder, rollout_length)-min(T_remainder, dp_length)) * sum(discount^(t - t_trunc) * reg_est * (1 + T - t) / (1 + T - t_trunc) for t in t_trunc:T)
